...
def train(
    ...
    dnn.fit(train_generator,
        ...
        steps_per_epoch    = len(train_idxs) * 250 // batch_size, 
        epochs             = max_epochs,
        callbacks          = callbacks,
        verbose            = 2)
...
if __name__ == "__main__":
    ...
    s_dir = path.join(WORK_DIR, "segmentations/")
    m_dir = path.join(WORK_DIR, "segmentations/manual/")
    ...
    img_files, seg_files = [], []
    for bn in sorted([path.basename(s) for s in all_images]):
        img_path = path.join(IMAGE_DIR, bn)
        bn_no_ext = bn[:-7]
        matching_segs = glob(path.join(m_dir, bn_no_ext + "*.nii.gz"))
        if len(matching_segs) < 1:
            continue
        seg_path = matching_segs[0]

        if path.exists(img_path) and path.exists(seg_path):
            img_files.append(img_path)
            seg_files.append(seg_path)
            print_("> Found matching image and segmentation for:", bn)
            print_(">> segmentation for:", bn, path.basename(seg_path))
    
    # test_img_paths = [img_files.pop() for _ in range(10)]
    # test_seg_paths = [seg_files.pop() for _ in range(10)]
    # test_imgs = [sitk.ReadImage(i) for i in test_img_paths]
    # test_segs = [sitk.ReadImage(s, sitk.sitkUInt8) for s in test_seg_paths]
