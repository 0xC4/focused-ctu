...
EXPORT_URETER_REPONSE_MAPS = False
...
def predict(input_s, 
    ...
    while x <= possible_offsets[0]:
        x_list += [x]
        x += max(min(window_size[0]//3, possible_offsets[0]-x), 1)
    while y <= possible_offsets[1]:
        y_list += [y]
        y += max(min(window_size[1]//3, possible_offsets[1]-y), 1)
    while z <= possible_offsets[2]:
        z_list += [z]
        z += max(min(window_size[2]//3, possible_offsets[2]-z), 1)
    ...
    # Return the ureter response map for debugging
    ureter_map_n = full_prediction[..., 2]
    ureter_map_n = np.round(ureter_map_n, 2)
    ureter_map_s = sitk.GetImageFromArray(ureter_map_n.T)
    ureter_map_s.CopyInformation(original_s)
    ...
    return masked_image_s, segmentation_s, mask_s, ureter_map_s
...
if __name__ == "__main__":
    ...
    for i, filename in enumerate(input_files):
        ...
        # Get the prediction
        masked_image, segmentation, mask, ureter = predict(
            input_s=input_s, 
            models=models, 
            window_size=(192, 160, 128),
            resample_min_shape=(192, 160, 128),
            resample_spacing=(1., 1., 2.),
        ...
        sitk.WriteImage(ureter, path.join(ureter_dir, bn))
        print_("> Done.")
